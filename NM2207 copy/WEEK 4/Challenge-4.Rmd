...
**Solution:** There are multiple variables in the column United States. It can be made more tidy if they is only one variable in a column. Under the percentage column, there are values with both % and without, making it hard to compute during code.It can be made more tidy if the values are presented in % form too.